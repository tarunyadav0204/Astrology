...
export const THEMES = {
  ...
  // iOS App Store variant: white background, black foreground only
  classic: {
    background: '#ffffff',
    ...
    gradientMid: '#f5f5f5',
    gradientEnd: '#eeeeee',
    gradientAccent: '#f5f5f5',
    cardBackground: '#ffffff',
    cardBorder: 'rgba(0, 0, 0, 0.22)',
    statusBarStyle: 'dark-content',
    strokeMuted: 'rgba(0, 0, 0, 0.2)',
    strokeStrong: 'rgba(0, 0, 0, 0.35)',
  },
};

export const ThemeProvider = ({ children, initialTheme }) => {
  const [theme, setTheme] = useState(initialTheme === 'light' || initialTheme === 'dark' || initialTheme === 'classic' ? initialTheme : 'dark');
  ...
  const toggleTheme = async () => {
    // Classic (astrology-only) app: no theme toggle, keep classic
  ...
  const isAndroidLight = Platform.OS === 'android' && (theme === 'light' || theme === 'classic');
  ...
  const colors = isAndroidLight
    ? {
        ...
        cardBorder: theme === 'classic' ? baseColors.cardBorder : 'rgba(249, 115, 22, 0.12)',
      }
    : baseColors;
