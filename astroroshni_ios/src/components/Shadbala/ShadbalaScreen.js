...
const ShadbalaScreen = ({ route, navigation }) => {
  ...
  if (loading) {
    return (
      <>
        ...
        <LinearGradient colors={isClassic ? [colors.background, colors.backgroundSecondary] : [colors.gradientStart, colors.gradientMid]} style={styles.container}>
  ...
  return (
    <>
      {isClassic && <StatusBar barStyle="dark-content" backgroundColor={colors.background} />}
      <LinearGradient colors={isClassic ? [colors.background, colors.backgroundSecondary] : [colors.gradientStart, colors.gradientMid, colors.gradientEnd]} style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <Text style={[styles.title, { color: colors.text }]}>{t('shadbalaAnalysis', 'Shadbala Analysis')}</Text>
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>{t('classicalPlanetaryStrength', 'Classical Planetary Strength (7 Visible Planets)')}</Text>
          ...
          {Object.keys(filteredPlanetData).map(planet => (
            <TouchableOpacity
              ...
              style={[
                ...
                selectedPlanet === planet && (isClassic ? { backgroundColor: colors.surface, borderColor: colors.primary, borderWidth: 2 } : { backgroundColor: colors.primary + '40', borderColor: colors.primary })
              ]}>
        ...
        {selected && (
          <>
            <View style={[styles.card, { backgroundColor: colors.cardBackground, borderColor: colors.cardBorder }]}>
              ...
              <View style={styles.strengthRow}>
                <View style={styles.strengthItem}>
                  ...
                  <Text style={[styles.strengthLabel, { color: colors.textSecondary }]}>{t('rupas', 'Rupas')}</Text>
                </View>
                ...
                <View style={styles.strengthItem}>
                  ...
                  <Text style={[styles.strengthLabel, { color: colors.textSecondary }]}>{t('points', 'Points')}</Text>
                </View>
              </View>
            ...
            <View style={[styles.card, { backgroundColor: colors.cardBackground, borderColor: colors.cardBorder }]}>
              <Text style={[styles.sectionTitle, { color: colors.text }]}>{t('strengthComponents', 'Strength Components')}</Text>
              {Object.entries(selected.components).map(([key, value]) => (
                <View key={key} style={[styles.componentRow, { borderBottomColor: colors.cardBorder }]}>
                  <Text style={[styles.componentName, { color: colors.textSecondary }]}>{t(`components.${key}`, formatComponentName(key))}</Text>
            ...
            {selected.detailed_breakdown && (
              <>
                <View style={[styles.card, { backgroundColor: colors.cardBackground, borderColor: colors.cardBorder }]}>
                  <Text style={[styles.sectionTitle, { color: colors.text }]}>{t('sthanaBalaBreakdown', 'Sthana Bala Breakdown')}</Text>
