...
const FortressWheel = ({ kotaData, colors, isClassic, onPlanetPress }) => {
  const getSectionColor = (section) => {
    if (isClassic) {
      switch (section) {
        case 'Stambha': return colors.textTertiary;
        case 'Madhya': return colors.textSecondary;
        case 'Prakaara': return colors.primary;
        case 'Bahya': return colors.text;
  ...
  const getPlanetColor = (planetData, data) => {
    const planetName = planetData.planet;
    if (planetName === data.kota_swami) return isClassic ? colors.primary : '#FFD700';
    if (planetName === data.kota_paala) return isClassic ? colors.textSecondary : '#4A90E2';
    if (planetData.is_benefic) return isClassic ? colors.text : colors.success;
    return isClassic ? colors.textTertiary : colors.error;
  };
  ...
  const renderPlanetInSection = (planets, section, radius, centerX, centerY) => {
    return planets.map((planetData, index) => {
      ...
      return (
        ...
        >
          <Text style={[styles.planetText, isClassic && { color: colors.background }]}>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.legend}>
        ...
        <View style={styles.legendRow}>
          <View style={[styles.legendColor, { backgroundColor: isClassic ? colors.primary : '#FFD700' }]} />
        ...
        <View style={styles.legendRow}>
          <View style={[styles.legendColor, { backgroundColor: isClassic ? colors.textSecondary : '#4A90E2' }]} />
          <Text style={[styles.legendText, { color: colors.textSecondary }]}>
            Kota Paala (Guard)
          </Text>
        </View>
        ...
        <View style={styles.legendRow}>
          <View style={[styles.legendColor, { backgroundColor: isClassic ? colors.text : colors.success }]} />
          <Text style={[styles.legendText, { color: colors.textSecondary }]}>
            Benefics (Protective)
          </Text>
        </View>
        ...
        <View style={styles.legendRow}>
          <View style={[styles.legendColor, { backgroundColor: isClassic ? colors.textTertiary : colors.error }]} />
          <Text style={[styles.legendText, { color: colors.textSecondary }]}>
