...
const KotaChakraScreen = ({ route, navigation }) => {
  ...
  const getStatusColor = (status) => {
    if (isClassic) {
      switch (status) {
        case 'High Vulnerability': return colors.textTertiary;
        case 'Moderate Caution': return colors.textSecondary;
        case 'Protected': return colors.text;
  ...
  if (loading) {
    return (
      <>
        ...
        <LinearGradient colors={isClassic ? [colors.background, colors.backgroundSecondary] : [colors.gradientStart, colors.gradientEnd]} style={styles.container}>
    ...
    <>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          <TouchableOpacity 
            style={[styles.backButton, isClassic && { backgroundColor: colors.surface }]}
        ...
        {kotaData && (
          <View style={[styles.statusCard, { backgroundColor: colors.cardBackground, borderColor: colors.cardBorder }]}>
            ...
            <View style={styles.protectionMeter}>
              ...
              <View style={[styles.meterBar, isClassic && { backgroundColor: colors.surface }]}>
                <View 
                  style={[
                    styles.meterFill, 
                    { 
        ...
        <View style={styles.actionButtons}>
          <TouchableOpacity
            style={[styles.actionButton, styles.goodButton, { backgroundColor: isClassic ? colors.primary : colors.success }, isClassic && { borderWidth: 1, borderColor: colors.cardBorder }]}
            onPress={() => fetchPeriods('good')}
          >
            <Text style={[styles.actionButtonText, isClassic && { color: colors.background }]}>🟢 Good Periods</Text>
          </TouchableOpacity>
          
          <TouchableOpacity
            style={[styles.actionButton, styles.vulnerableButton, { backgroundColor: isClassic ? colors.surface : colors.error }, isClassic && { borderWidth: 1, borderColor: colors.cardBorder }]}
            onPress={() => fetchPeriods('vulnerable')}
          >
            <Text style={[styles.actionButtonText, isClassic && { color: colors.text }]}>🔴 Vulnerable</Text>
          </TouchableOpacity>
        </View>
