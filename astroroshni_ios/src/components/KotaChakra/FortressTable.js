...
const FortressTable = ({ kotaData, colors, isClassic }) => {
  const getThreatLevel = (planets) => {
    ...
    if (isClassic) {
      if (malefics.length > 0 && benefics.length === 0) return { level: 'High', color: colors.textTertiary };
      if (malefics.length > 0 && benefics.length > 0) return { level: 'Moderate', color: colors.textSecondary };
      if (benefics.length > 0) return { level: 'Protected', color: colors.text };
    ...
    return (
      <View key={zone} style={[styles.tableRow, { borderColor: colors.cardBorder }]}>
        <View style={[styles.zoneCell, { backgroundColor: zoneColor }]}>
          <Text style={[styles.zoneText, { color: isClassic ? colors.background : '#ffffff' }]}>{zone}</Text>
        </View>
