...
const PeriodsModal = ({ visible, onClose, type, data, colors, isClassic }) => {
  const periods = type === 'good' ? data.good_periods : data.vulnerable_periods;
  
  const getStatusColor = (vulnerabilityScore) => {
    if (isClassic) {
      if (vulnerabilityScore >= 6) return colors.textTertiary;
      if (vulnerabilityScore >= 3 || vulnerabilityScore >= 2) return colors.textSecondary;
      return colors.text;
    }
    ...
    <View style={[styles.periodCard, { backgroundColor: colors.cardBackground, borderColor: colors.cardBorder }]}>
      <View style={styles.periodHeader}>
        ...
        <View style={[styles.scoreBadge, { backgroundColor: getStatusColor(item.vulnerability_score) }]}>
          <Text style={[styles.scoreText, isClassic && { color: colors.background }]}>
      ...
      <LinearGradient colors={isClassic ? [colors.background, colors.backgroundSecondary] : [colors.gradientStart, colors.gradientEnd]} style={styles.container}>
        <View style={styles.header}>
          ...
          <TouchableOpacity
            ...
            onPress={onClose}
          >
            <Text style={[styles.closeButtonText, isClassic && { color: colors.background }]}>✕</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={[styles.footer, isClassic && { backgroundColor: colors.backgroundSecondary, borderTopWidth: 1, borderTopColor: colors.cardBorder }]}>
          <View style={styles.legendContainer}>
            ...
            <View style={styles.legendItems}>
              <View style={styles.legendItem}>
                <View style={[styles.legendDot, { backgroundColor: isClassic ? colors.text : colors.success }]} />
                <Text style={[styles.legendText, { color: colors.textSecondary }]}>
                  0-1: Protected
                </Text>
              </View>
              <View style={styles.legendItem}>
                <View style={[styles.legendDot, { backgroundColor: isClassic ? colors.textSecondary : colors.warning }]} />
                <Text style={[styles.legendText, { color: colors.textSecondary }]}>
                  2-5: Caution
                </Text>
              </View>
              <View style={styles.legendItem}>
                <View style={[styles.legendDot, { backgroundColor: isClassic ? colors.textTertiary : colors.error }]} />
                <Text style={[styles.legendText, { color: colors.textSecondary }]}>
                  6+: High Risk
                </Text>
              </View>
