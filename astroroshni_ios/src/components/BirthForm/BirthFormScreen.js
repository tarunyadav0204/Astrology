...
export default function BirthFormScreen({ navigation, route }) {
  ...
  const pickerSheetGradient = isClassic
    ? [colors.cardBackground, colors.backgroundSecondary]
    : theme === 'dark'
      ? ['rgba(255, 255, 255, 0.15)', 'rgba(255, 255, 255, 0.05)']
      : ['rgba(249, 115, 22, 0.22)', 'rgba(249, 115, 22, 0.1)'];
  const pickerSheetHeaderBorder = isClassic
    ? colors.cardBorder
    : theme === 'dark'
      ? 'rgba(255, 255, 255, 0.12)'
      : 'rgba(249, 115, 22, 0.28)';
  const pickerOverlayTint = isClassic
    ? 'rgba(0, 0, 0, 0.45)'
    : theme === 'dark'
      ? 'rgba(0, 0, 0, 0.65)'
      : 'rgba(28, 25, 23, 0.45)';
  const datePickerThemeVariant = isClassic ? 'light' : theme === 'dark' ? 'dark' : 'light';
      ...
      <LinearGradient colors={isClassic ? [colors.background, colors.backgroundSecondary] : (theme === 'dark' ? [colors.gradientStart, colors.gradientMid, colors.gradientEnd, colors.primary] : [colors.gradientStart, colors.gradientStart, colors.gradientStart, colors.gradientStart])} style={styles.gradient}>
        <SafeAreaView style={styles.safeArea}>
          <KeyboardAvoidingView style={styles.keyboardAvoid} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            ...
            {/* Date Picker — spinner wheels; sheet styled to match birth-form cards */}
            <Modal visible={showDatePicker} transparent animationType="slide">
              <View style={[styles.modalOverlay, { backgroundColor: pickerOverlayTint }]}>
