...
const ChartWidget = forwardRef(({ title, chartType, chartData, birthData, lagnaChartData, defaultStyle = 'north', disableSwipe = false, hideHeader = false, cosmicTheme = false, onOpenDasha, onNavigateToTransit, division, navigation, onHousePress }, ref) => {
  ...
  const QuickActionButton = ({ icon, label, onPress, active, primary }) => {
    const iconColor = isClassic
      ? (primary ? colors.background : colors.text)
      : (primary ? '#fff' : (isLight ? '#1e293b' : '#fff'));
    const textColor = isClassic
      ? (primary ? colors.background : colors.text)
      : (primary ? '#fff' : (isLight ? '#1e293b' : '#fff'));
    return (
      <TouchableOpacity 
        style={[
          ...
          isLight && !isClassic && styles.quickActionButtonLight,
          isClassic && { backgroundColor: colors.surface, borderColor: colors.cardBorder },
          active && !isClassic && (isLight ? styles.quickActionButtonActiveLight : styles.quickActionButtonActive),
          active && isClassic && { backgroundColor: colors.backgroundSecondary, borderColor: colors.primary },
          primary && (isClassic ? { backgroundColor: colors.primary, borderColor: colors.primary } : styles.quickActionButtonPrimary)
        ]} 
        onPress={onPress}
      >
        ...
        <Text style={[
          ...
          primary && !isClassic && styles.quickActionTextPrimary
        ]}>
          {label}
        </Text>
      </TouchableOpacity>
          ...
          <>
            <View style={[styles.floatingControls, currentChartType === 'transit' && styles.floatingControlsTransit]}>
              <TouchableOpacity
                ...
                style={[
                  ...
                  showDegreeNakshatra && styles.floatingButtonActive,
                  isClassic && { backgroundColor: colors.surface, borderColor: colors.cardBorder },
                  isClassic && showDegreeNakshatra && { backgroundColor: colors.primary, borderColor: colors.primary },
                ]}
              >
                <Ionicons name={showDegreeNakshatra ? "eye" : "eye-off"} size={18} color={isClassic ? colors.text : '#fff'} />
              </TouchableOpacity>
              <TouchableOpacity
                onPress={toggleStyle}
                style={[styles.floatingButton, isClassic && { backgroundColor: colors.surface, borderColor: colors.cardBorder }]}
              >
                <Text style={[styles.floatingButtonText, isClassic && { color: colors.text }]}>{chartStyle === 'north' ? 'S' : 'N'}</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                onPress={() => navigation?.navigate('AshtakvargaOracle')} 
                style={[styles.floatingButton, isClassic && { backgroundColor: colors.surface, borderColor: colors.cardBorder }]}
              >
                <Ionicons name="grid-outline" size={18} color={isClassic ? colors.text : '#fff'} />
              </TouchableOpacity>
              <TouchableOpacity 
            ...
            <View style={styles.infoButtonContainer}>
              <TouchableOpacity
                ...
                style={[styles.infoButton, isClassic && { backgroundColor: colors.surface, borderColor: colors.cardBorder }]}
              >
                <Ionicons name="information-circle-outline" size={20} color={isClassic ? colors.text : '#fff'} />
              </TouchableOpacity>
            </View>
          </>
        )}
