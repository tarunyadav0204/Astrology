...
const FructificationView = ({ data, theme, colors }) => {
    ...
    const surface = isClassic ? colors.surface : (isDark ? '#1a1030' : '#ffffff');
    const surfaceMuted = isClassic ? colors.backgroundSecondary : (isDark ? '#241540' : '#f1f5f9');
    const border = isClassic ? colors.cardBorder : (isDark ? 'rgba(255,255,255,0.22)' : 'rgba(100, 116, 139, 0.28)');
    const bodyText = isClassic ? colors.text : (isDark ? '#f8fafc' : '#0f172a');
    const mutedText = isClassic ? colors.textSecondary : (isDark ? '#cbd5e1' : '#334155');
    const subtleText = isClassic ? colors.textSecondary : (isDark ? '#94a3b8' : '#64748b');
    // Avoid orange link text over the orange end of the gradient.
    const linkColor = isClassic ? colors.primary : (isDark ? '#f8fafc' : colors.primary);
    const useDarkTone = isDark && !isClassic;
    ...
    const renderHowSteps = (how, accent) => {
        ...
        const passTone = toneUi('supportive', useDarkTone);
        const failTone = toneUi('challenging', useDarkTone);
        return (
    ...
    const renderHouseCard = (row, soft = false) => {
        ...
        const tone = soft ? toneUi('neutral', useDarkTone) : toneUi(row.tone, useDarkTone);
    ...
    return (
        <View>
            ...
            <View style={[styles.fructScopeBar, { backgroundColor: isClassic ? colors.backgroundSecondary : (isDark ? 'rgba(0,0,0,0.45)' : '#f1f5f9'), borderColor: border }]}>
                {[
                    ...
                    return (
                        <TouchableOpacity
                            ...
                            activeOpacity={0.9}
                        >
                            <Text style={[styles.fructScopeChipText, { color: active ? (isClassic ? colors.background : '#fff') : bodyText }]}>
                                {tab.label}
                            </Text>
            ...
            {gate.prana_fallback ? (
                <View style={[styles.fructNotice, { backgroundColor: useDarkTone ? 'rgba(56,189,248,0.18)' : '#e0f2fe', borderColor: useDarkTone ? '#38bdf8' : '#7dd3fc' }]}>
                    <Ionicons name="information-circle-outline" size={16} color={useDarkTone ? '#38bdf8' : '#0369a1'} />
            ...
            <TouchableOpacity
                ...
                activeOpacity={0.88}
            >
                <View style={[styles.fructCalcIcon, { backgroundColor: useDarkTone ? 'rgba(253,186,116,0.2)' : 'rgba(249,115,22,0.14)' }]}>
                ...
                return groups.map((group) => (
                    <View key={group.subject} style={{ marginBottom: 14 }}>
                        <View style={styles.fructSubjectHead}>
                            <View style={[styles.fructSubjectIcon, { backgroundColor: useDarkTone ? 'rgba(253,186,116,0.2)' : 'rgba(249,115,22,0.14)' }]}>
                                <Ionicons
                        ...
                        {group.items.map((item) => {
                            const tone = toneUi(item.outcome_tone, useDarkTone);
                            return (
                                <View
            ...
            >
                <SafeAreaView style={styles.safeArea}>
                    ...
                    <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
                        ...
                        <View style={[styles.contentCard, { backgroundColor: isClassic ? colors.cardBackground : (theme === 'dark' ? (activeTab === 'results' ? '#140c24' : 'rgba(255, 255, 255, 0.05)') : 'rgba(255, 255, 255, 0.9)'), borderWidth: isClassic ? 1 : 0, borderColor: isClassic ? colors.cardBorder : undefined }]}>
                            {renderContent()}
