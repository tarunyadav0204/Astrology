    ...
    return (
        <ScrollView showsVerticalScrollIndicator={false}>
            {Object.entries(data).map(([house, significators]) => (
                <View key={house} style={[styles.significatorCard, { backgroundColor: isClassic ? colors.surface : (theme === 'dark' ? 'rgba(255, 255, 255, 0.05)' : 'rgba(249, 115, 22, 0.05)'), borderColor: isClassic ? colors.cardBorder : undefined }]}>
                    ...
                    <View style={styles.significatorChips}>
                        {significators.map((sig, idx) => (
                            <View key={idx} style={[styles.significatorChip, { backgroundColor: isClassic ? colors.backgroundSecondary : (colors.primary + '20'), borderColor: colors.primary }]}>
    ...
    return (
        <ScrollView showsVerticalScrollIndicator={false}>
            {Object.entries(data).map(([planet, houses]) => (
                <View key={planet} style={[styles.significatorCard, { backgroundColor: isClassic ? colors.surface : (theme === 'dark' ? 'rgba(255, 255, 255, 0.05)' : 'rgba(249, 115, 22, 0.05)'), borderColor: isClassic ? colors.cardBorder : undefined }]}>
    ...
    const renderStep = (num, label, lord, houses) => (
        <View style={styles.stepContainer}>
            <View style={styles.stepIndicator}>
                ...
                {num < 4 && <View style={[styles.stepLine, { backgroundColor: isClassic ? colors.cardBorder : (colors.primary + '30') }]} />}
            </View>
                ...
                <View style={styles.stepDetails}>
                    ...
                    <View style={styles.stepHouses}>
                        {houses.length > 0 ? houses.map((h, idx) => (
                            <View key={idx} style={[styles.miniHouseChip, { backgroundColor: isClassic ? colors.backgroundSecondary : (colors.primary + '10') }]}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
            {Object.entries(data).map(([planet, steps]) => (
                <View key={planet} style={[styles.fourStepCard, { 
                    ...
                    borderColor: isClassic ? colors.cardBorder : (colors.primary + '20')
                }]}>
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
                                {['planets', 'cusps', 'significators', 'planetSignificators', 'fourStep'].map((tab) => (
                                    ...
                                    >
                                        ...
                                        >
                                            <Text style={[styles.tabText, { color: activeTab === tab ? (isClassic ? colors.background : '#fff') : colors.text }]}>
                                                {tab === 'planets' ? 'Planets' : 
                                                 tab === 'cusps' ? 'Cusps' : 
