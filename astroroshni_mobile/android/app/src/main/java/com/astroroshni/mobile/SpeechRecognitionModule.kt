...
class SpeechRecognitionModule(private val reactContext: ReactApplicationContext) :
  ...
  companion object {
    // Partial results still reach React Native immediately. These endpoint hints
    // only keep a natural thinking pause from closing the utterance too early.
    private const val MINIMUM_LISTENING_MILLIS = 900L
    private const val COMPLETE_SILENCE_MILLIS = 3200L
    private const val POSSIBLY_COMPLETE_SILENCE_MILLIS = 2400L
    private const val PARTIAL_STABLE_MILLIS = 3000L
    private const val MAX_LISTENING_MILLIS = 30000L
  }
