          ...
          <div class="container">
            ...
            <div class="footer">
              ...
              <div class="disclaimer-text">
                ...
                It is <strong>not</strong> a substitute for professional medical, legal, psychological or financial advice. For any decisions regarding your
                health, finances, career, or legal matters, please consult a qualified professional.
              </div>
              <div style="margin-top: 16px;">Generated by AstroRoshni App — Your Personal Vedic Astrology Guide</div>
            </div>
          </div>
        </body>
      </html>`;
...
const htmlParagraph = (text) => `<p>${escapeHtml(cleanInlineMarkup(text))}</p>`;

export const generateRelationshipReportPDF = async ({
  personOne,
  personTwo,
  result,
  premiumReport,
  ...
  try {
    ...
    const metrics = [
      ['Overall score', result?.overall_score?.percentage != null ? `${Math.round(result.overall_score.percentage)}%` : '--'],
      ['Guna Milan', result?.guna_milan?.effective_total_score != null ? `${result.guna_milan.effective_total_score}/36` : '--'],
      ['Manglik', result?.manglik_analysis?.compatibility?.status || '--'],
      ['Current timing', result?.timing_overlay?.shared?.current_window?.climate || '--'],
    ];
    ...
    const html = `
      ...
      <html>
        <head>
          <meta charset="utf-8" />
          <style>
            body { margin: 0; padding: 28px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: #2d1b4e; background: #fff8f5; }
            .page { max-width: 760px; margin: 0 auto; background: #fff; border-radius: 20px; padding: 28px; border: 1px solid #f3d3c3; }
            .header { text-align: center; margin-bottom: 24px; }
            .logo { width: 54px; height: 54px; object-fit: contain; margin-bottom: 8px; }
            h1 { margin: 0; color: #7c2d12; font-size: 26px; }
            .subtitle { color: #7c2d12; opacity: 0.78; margin-top: 8px; }
            .pair { display: flex; gap: 12px; margin: 22px 0; }
            .person { flex: 1; background: #fff7ed; border-radius: 14px; padding: 14px; border: 1px solid #fed7aa; }
            .person-title { font-weight: 800; color: #9a3412; margin-bottom: 6px; }
            .meta { color: #7c2d12; font-size: 13px; line-height: 1.5; }
            .metrics { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin: 18px 0 24px; }
            .metric { background: #fdf2f8; border: 1px solid #fbcfe8; border-radius: 14px; padding: 12px; }
            .metric-value { font-size: 18px; font-weight: 900; color: #9d174d; }
            .metric-label { margin-top: 4px; font-size: 12px; color: #831843; }
            .verdict { background: #fff7ed; border-left: 4px solid #f97316; padding: 14px; border-radius: 12px; margin-bottom: 20px; color: #431407; line-height: 1.55; }
            .section { margin-top: 20px; padding-top: 18px; border-top: 1px solid #fed7aa; }
            h2 { font-size: 18px; color: #7c2d12; margin: 0 0 8px; }
            h3 { font-size: 13px; color: #ea580c; margin: 14px 0 6px; text-transform: uppercase; letter-spacing: 0.4px; }
        ...
        <body>
          <div class="page">
            <div class="header">
              ${optionLogoUri ? `<img src="${optionLogoUri}" class="logo" />` : ''}
              <h1>Premium Relationship Report</h1>
              <div class="subtitle">Generated by AstroRoshni</div>
            </div>
            <div class="pair">
              <div class="person">
                <div class="person-title">${escapeHtml(personOne?.name || 'Person 1')}</div>
                <div class="meta">${escapeHtml([personOne?.date, personOne?.time, personOne?.place].filter(Boolean).join(' • '))}</div>
              </div>
              <div class="person">
                <div class="person-title">${escapeHtml(personTwo?.name || 'Person 2')}</div>
                <div class="meta">${escapeHtml([personTwo?.date, personTwo?.time, personTwo?.place].filter(Boolean).join(' • '))}</div>
              </div>
            </div>
            <div class="metrics">
              ${metrics.map(([label, value]) => `<div class="metric"><div class="metric-value">${escapeHtml(value)}</div><div class="metric-label">${escapeHtml(label)}</div></div>`).join('')}
            </div>
            <div class="verdict">
              ${htmlParagraph(premiumReport?.compatibility_verdict || result?.recommendation?.recommendation || 'Relationship report generated.')}
            </div>
            ${sections.map((section) => `
              <div class="section">
                <h2>${escapeHtml(cleanInlineMarkup(section.title || section.key || 'Relationship insight'))}</h2>
                ${section.static_summary ? htmlParagraph(section.static_summary) : ''}
                ${section.ai_interpretation ? htmlParagraph(section.ai_interpretation) : ''}
                ${section.practical_meaning ? `<h3>Practical meaning</h3>${htmlParagraph(section.practical_meaning)}` : ''}
                ${section.decision_guidance ? `<h3>Decision guidance</h3>${htmlParagraph(section.decision_guidance)}` : ''}
              </div>
            `).join('')}
            ${premiumReport?.final_summary ? `<div class="section"><h2>Final summary</h2>${htmlParagraph(premiumReport.final_summary)}</div>` : ''}
            ${(premiumReport?.priority_actions || []).length ? `<div class="section"><h2>Priority actions</h2><ul>${premiumReport.priority_actions.map((item) => `<li>${escapeHtml(cleanInlineMarkup(item))}</li>`).join('')}</ul></div>` : ''}
            <div class="footer">
              This report is based on traditional Vedic astrological analysis and is intended for spiritual insight and personal reflection only.
              It is not a substitute for professional medical, legal, psychological, or financial advice.
            </div>
          </div>
        </body>
      </html>
