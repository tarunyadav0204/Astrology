...
import {
  ...
  translateHouseInterpretation,
  translateHouseVerdictLabel,
  translateOccupantRole,
} from '../../utils/translateHouseInsight';
...
function localizeNakshatraName(t, nakshatraName) {
  ...
  const key = String(nakshatraName).replace(/\s+/g, '_');
...
export default function ChartScreen({ navigation, route, onHeaderStateChange }) {
  ...
  const sectionSurface = { backgroundColor: colors.surfaceMuted, borderColor: colors.cardBorder };
  const nestedCard = { backgroundColor: colors.surface };
  const warningSurface = { backgroundColor: withAlpha(colors.warning, '22'), borderColor: colors.cardBorder };
  const dangerSurface = { backgroundColor: withAlpha(colors.error, '22'), borderColor: colors.cardBorder };
      ...
      <View style={[styles.container, webFlexFix]}>
        ...
        <Root {...rootProps}>
          ...
          <View style={styles.compactHeader}>
            <TouchableOpacity onPress={() => navigation.goBack()} style={[styles.closeButton, { backgroundColor: isDark ? 'rgba(255, 255, 255, 0.15)' : 'rgba(24, 24, 27, 0.08)' }]}>
            ...
            <View style={[styles.chartPosition, { backgroundColor: isDark ? 'rgba(255, 255, 255, 0.15)' : 'rgba(24, 24, 27, 0.08)' }]}>
            ...
            <View style={styles.loadingContainer}>
              <View style={styles.loadingContent}>
                <View style={styles.loadingOrb}>
                  {isWhiteTheme ? (
                    <View style={[styles.loadingOrbGradient, { backgroundColor: colors.backgroundTertiary }]}>
                  ...
                  >
                    ...
                    <View style={[styles.drawerSection, sectionSurface]}>
                      <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                        {t('chartScreen.houseDrawer.significance', 'Significance')}
                      </Text>
                    ...
                    {mudakkuAnalysis && selectedHouse.rashiIndex === mudakkuAnalysis?.mudakku_point?.sign && (
                      <View style={[styles.drawerSection, warningSurface]}>
                        <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                          {t('chartScreen.houseDrawer.mudakku', 'Mudakku / Modakku')}
                        </Text>
                        <Text style={[styles.sectionDesc, { color: colors.text }]}>
                          {t('chartScreen.houseDrawer.mudakkuDesc', 'This house contains the Mudakku point for your chart.')}
                        </Text>

                        <View style={styles.mudakkuDetailGrid}>
                          <View style={[styles.mudakkuDetailCard, nestedCard]}>
                            <Text style={[styles.mudakkuDetailLabel, { color: colors.textSecondary }]}>
                              {t('chartScreen.houseDrawer.sunNakshatra', 'Sun Nakshatra')}
                            </Text>
                            <Text style={[styles.mudakkuDetailValue, { color: colors.text }]}>
                              {localizeNakshatraName(t, mudakkuAnalysis.sun_nakshatra?.name)}
                            </Text>
                          </View>

                          <View style={[styles.mudakkuDetailCard, nestedCard]}>
                            <Text style={[styles.mudakkuDetailLabel, { color: colors.textSecondary }]}>
                              {t('chartScreen.houseDrawer.countToMula', 'Count to Mula')}
                            </Text>
                            <Text style={[styles.mudakkuDetailValue, { color: colors.text }]}>
                        ...
                        <Text style={[styles.mudakkuNote, { color: colors.textSecondary }]}>
                          {t('chartScreen.houseDrawer.landingNakshatra', {
                            nakshatra: localizeNakshatraName(t, mudakkuAnalysis.mudakku_nakshatra?.name),
                            rashi: t(`signs.${mudakkuAnalysis.mudakku_rashi}`, mudakkuAnalysis.mudakku_rashi || '-'),
                            defaultValue: 'Landing nakshatra: {{nakshatra}} · Rashi: {{rashi}}',
                          })}
                        </Text>
                        <Text style={[styles.mudakkuNote, { color: colors.textSecondary }]}>
                          {mudakkuAnalysis.is_split_nakshatra
                            ? t('chartScreen.houseDrawer.splitRule', 'Split nakshatra rule applied.')
                            : t('chartScreen.houseDrawer.singleLanding', 'Single sign landing.')}
                        </Text>
                      </View>
                    )}
                    ...
                    {(gandantaHouseMatches.planets.length > 0 || (selectedHouse.houseNum === 1 && gandantaAnalysis?.lagna_gandanta?.is_gandanta) || (selectedHouse.houseNum === 1 && gandantaHouseMatches.lagna?.is_gandanta)) && (
                      <View style={[styles.drawerSection, dangerSurface]}>
                        <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                          {t('chartScreen.houseDrawer.gandamoola', 'Gandamoola (Gandanta)')}
                        </Text>
                        <Text style={[styles.sectionDesc, { color: colors.text }]}>
                          {selectedHouse.houseNum === 1 && (gandantaAnalysis?.lagna_gandanta?.is_gandanta || gandantaHouseMatches.lagna?.is_gandanta)
                            ? t('chartScreen.houseDrawer.gandantaLagnaHouse', {
                                name: gandantaAnalysis?.lagna_gandanta?.gandanta_info?.gandanta_name
                                  || gandantaHouseMatches.lagna?.gandanta_info?.gandanta_name
                                  || 'junction',
                                defaultValue: 'This is a Gandanta Lagna house ({{name}}).',
                              })
                            : t('chartScreen.houseDrawer.gandantaPlanets', 'This house contains one or more planets in Gandanta.')}
                        </Text>

                        {gandantaHouseMatches.planets.length > 0 && (
                          <View style={styles.mudakkuDetailGrid}>
                            {gandantaHouseMatches.planets.map(({ planet, gandanta_info }, idx) => (
                              <View key={`${planet?.name || 'planet'}-${idx}`} style={[styles.mudakkuDetailCard, nestedCard]}>
                                <Text style={[styles.mudakkuDetailLabel, { color: colors.textSecondary }]}>
                                  {planet?.name
                                    ? t(`home.planet_names.${planet.name}`, planet.name)
                                    : t('chartScreen.houseDrawer.planet', 'Planet')}
                                </Text>
                                <Text style={[styles.mudakkuDetailValue, { color: colors.text }]}>
                                  {gandanta_info?.gandanta_name || 'Gandanta'}
                                </Text>
                                <Text style={[styles.mudakkuNote, { color: colors.textSecondary, marginTop: 4 }]}>
                                  {gandanta_info?.intensity
                                    ? t(
                                        `chartScreen.houseDrawer.${String(gandanta_info.intensity).toLowerCase()}`,
                                        gandanta_info.intensity,
                                      )
                                    : t('chartScreen.houseDrawer.medium', 'Medium')}{' '}
                                  · {gandanta_info?.distance_from_junction ?? '-'}°
                                </Text>
                              </View>
                            ))}
                        </View>
                        ...
                        {selectedHouse.houseNum === 1 && (gandantaAnalysis?.lagna_gandanta?.is_gandanta || gandantaHouseMatches.lagna?.is_gandanta) && (
                          <Text style={[styles.mudakkuNote, { color: colors.textSecondary }]}>
                            {t('chartScreen.houseDrawer.lagnaPoint', {
                              name: gandantaAnalysis?.lagna_gandanta?.gandanta_info?.gandanta_name
                                || gandantaHouseMatches.lagna?.gandanta_info?.gandanta_name
                                || 'Gandanta',
                              defaultValue: 'Lagna point: {{name}}',
                            })}
                          </Text>
                        )}
                        ...
                        {(gandantaAnalysis?.moon_gandanta?.is_gandanta || gandantaHouseMatches.moon?.is_gandanta) && currentChartIndex === 0 && (
                          <Text style={[styles.mudakkuNote, { color: colors.textSecondary }]}>
                            {t('chartScreen.houseDrawer.moonAlso', {
                              name: gandantaAnalysis?.moon_gandanta?.gandanta_info?.gandanta_name
                                || gandantaHouseMatches.moon?.gandanta_info?.gandanta_name
                                || 'Gandanta',
                              defaultValue: 'Moon also falls in {{name}}.',
                            })}
                          </Text>
                        )}
                      </View>
                        ...
                        <View style={styles.insightHeaderRow}>
                          ...
                          <View
                            style={[
                              ...
                              {
                                backgroundColor:
                                  houseInsight.verdict.key === 'strong' ? withAlpha(colors.success, '28')
                                  : houseInsight.verdict.key === 'mixed' ? withAlpha(colors.warning, '28')
                                  : houseInsight.verdict.key === 'active' ? withAlpha(colors.info, '28')
                                  : colors.surfaceMuted,
                                borderColor: colors.cardBorder,
                              },
                            ]}
                          >
                            ...
                            >
                              {translateHouseVerdictLabel(t, houseInsight.verdict?.label)}
                            </Text>
                          </View>
                        </View>
                    ...
                    {houseInsight?.support_factors?.length > 0 && (
                      <View style={[styles.drawerSection, sectionSurface]}>
                        <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                          {t('chartScreen.houseDrawer.supports', 'What supports this house')}
                        </Text>
                    ...
                    {houseInsight?.stress_factors?.length > 0 && (
                      <View style={[styles.drawerSection, sectionSurface]}>
                        <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                          {t('chartScreen.houseDrawer.pressure', 'What adds pressure')}
                        </Text>
                    ...
                    {houseInsight?.raw?.ashtakavarga?.sav && (
                      <View style={[styles.drawerSection, sectionSurface]}>
                        <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                          {t('chartScreen.houseDrawer.ashtakavarga', 'Ashtakavarga')}
                        </Text>
                        <View style={styles.ashtakavargaGrid}>
                          <View style={[styles.ashtakavargaCard, nestedCard]}>
                            ...
                            <Text style={[styles.ashtakavargaMeta, { color: colors.textSecondary }]}>
                              {translateAshtakClassification(
                                t,
                                houseInsight.raw.ashtakavarga.sav?.classification,
                              )}
                            </Text>
                          </View>
                          ...
                          {houseInsight.raw.ashtakavarga.lord_bav && (
                            <View style={[styles.ashtakavargaCard, nestedCard]}>
                              ...
                              <Text style={[styles.ashtakavargaMeta, { color: colors.textSecondary }]}>
                                {translateAshtakClassification(
                                  t,
                                  houseInsight.raw.ashtakavarga.lord_bav.classification,
                                )}
                              </Text>
                            </View>
                          )}
                        ...
                        <Text style={[styles.ashtakavargaExplain, { color: colors.textSecondary }]}>
                          {t('chartScreen.houseDrawer.savExplain', 'SAV shows the house’s total support. BAV shows how strongly the house lord contributes to this house.')}
                        </Text>

                        {houseInsight.raw.ashtakavarga.lord_bav?.strongest_houses?.length > 0 && (
                          <Text style={[styles.ashtakavargaDetail, { color: colors.text }]}>
                            {t('chartScreen.houseDrawer.strongestBav', {
                              planet: t(
                                `home.planet_names.${houseInsight.raw.ashtakavarga.lord_bav.planet}`,
                                houseInsight.raw.ashtakavarga.lord_bav.planet,
                              ),
                              houses: houseInsight.raw.ashtakavarga.lord_bav.strongest_houses.join(', '),
                              defaultValue: 'Strongest {{planet}} BAV houses: {{houses}}',
                            })}
                          </Text>
                        )}
                        ...
                        {houseInsight.raw.ashtakavarga.lord_bav?.weakest_houses?.length > 0 && (
                          <Text style={[styles.ashtakavargaDetail, { color: colors.text }]}>
                            {t('chartScreen.houseDrawer.weakestBav', {
                              planet: t(
                                `home.planet_names.${houseInsight.raw.ashtakavarga.lord_bav.planet}`,
                                houseInsight.raw.ashtakavarga.lord_bav.planet,
                              ),
                              houses: houseInsight.raw.ashtakavarga.lord_bav.weakest_houses.join(', '),
                              defaultValue: 'Weakest {{planet}} BAV houses: {{houses}}',
                            })}
                          </Text>
                        )}
                    ...
                    <View style={[styles.drawerSection, sectionSurface]}>
                      <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>
                        {t('chartScreen.houseDrawer.occupantPlanets', 'Occupant Planets')}
                      </Text>
                      {selectedHouse.planets && selectedHouse.planets.length > 0 ? (
                        selectedHouse.planets.map((planet, idx) => {
                          ...
                          <View key={idx} style={styles.planetRow}>
                            <View style={[styles.planetIconContainer, { backgroundColor: withAlpha(colors.primary, '15') }]}>
                                    ...
                                    style={[
                                      styles.occupantRoleChip,
                                      {
                                        backgroundColor: role === 'Yogi lord'
                                          ? withAlpha(colors.success, '28')
                                          : withAlpha(colors.error, '22'),
                                      },
                                    ]}
                                  >
                                    <Text style={[styles.occupantRoleChipText, { color: colors.text }]}>
                                      {translateOccupantRole(t, role)}
                                    </Text>
                                  </View>
                                ))}
                              </View>
                              <Text style={[styles.planetDetails, { color: colors.textSecondary }]}>
                                {t('chartScreen.houseDrawer.degreeInNakshatra', {
                                  degree: planet.formattedDegreeFull || planet.formattedDegree,
                                  nakshatra: localizeNakshatraName(t, planet.nakshatra),
                                  defaultValue: '{{degree}} in {{nakshatra}}',
                                })}
                                {planet.pada != null
                        ...
                        <Text style={[styles.emptySectionText, { color: colors.textTertiary }]}>
                          {t('chartScreen.houseDrawer.noPlanets', 'No planets occupy this house.')}
                        </Text>
                      )}
                    </View>
                      ...
                      {houseDrawerAspects.length > 0 ? (
                        houseDrawerAspects.map((row, idx) => (
                          <View key={`${row.planetName}-${idx}`} style={styles.aspectRow}>
                            ...
                            <View style={styles.planetInfo}>
                              <Text style={[styles.planetName, { color: colors.text }]}>
                                {t(`home.planet_names.${row.planetName}`, row.planetName)}
                  ...
                  <View style={[styles.drawerActions, {
                    ...
                    >
                      ...
                      <Text style={[styles.actionButtonText, { color: colors.text }]}>
                        {t('chartScreen.houseDrawer.ask', 'Ask')}
                      </Text>
                    </TouchableOpacity>
                    ...
                    {(houseInsight?.related_chart || houseInsight?.relatedChart) && currentChartIndex === 0 && (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          const related = houseInsight.related_chart || houseInsight.relatedChart;
                          const relatedIndex = chartTypes.findIndex((chart) => chart.id === related.id);
                          closeHouseDrawer();
                          if (relatedIndex >= 0) {
                      ...
                      >
                        ...
                        <Text style={[styles.actionButtonText, { color: colors.text }]}>
                          {t('chartScreen.houseDrawer.openRelated', {
                            id: (houseInsight.related_chart || houseInsight.relatedChart).id.toUpperCase(),
