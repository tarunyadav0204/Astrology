                                        ...
                                        continue;
                                            ...
                                            if (parsed.status === 'complete') {
                                                ...
                                                if (typeof parsed.response === 'string') {
                                                    const analysisSteps = parsed.analysis_steps || [];
                                                    if (analysisSteps.length > 0) {
                                                        // Animate analysis steps like mobile until final response is ready.
                                                        if (loadingTimer) clearInterval(loadingTimer);
                                                        loadingTimer = null;
                                                        stopStreaming = true;

                                                        const finalResponseText = parsed.response;
                                                        const finalTerms = parsed.terms || [];
                                                        const finalGlossary = parsed.glossary || {};
                                                        const finalSummaryImage = parsed.summary_image || null;
                                                        const finalMessageType = parsed.message_type || assistantMessage.message_type || 'answer';

                                                        assistantMessage = {
                                                            ...assistantMessage,
                                                            content: `⚙️ ${analysisSteps[0]}...`,
                                                            loadingMessage: null,
                                                            isProcessing: true,
                                                            isTyping: true,
                                                            terms: finalTerms,
                                                            glossary: finalGlossary,
                                                            summary_image: finalSummaryImage,
                                                        ...
                                                        const stepInterval = setInterval(() => {
                                                            ...
                                                            if (stepIndex < analysisSteps.length) {
                                                                setMessages(prev => {
                                                                    ...
                                                                    newMessages[idx] = {
                                                                        ...assistantMessage,
                                                                        content: `⚙️ ${analysisSteps[stepIndex]}...`,
                                                                    };
                                                                    return newMessages;
                                                                });
                                                            } else {
