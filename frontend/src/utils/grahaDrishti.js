 ...
 * Mirrors astroroshni_mobile/src/utils/grahaDrishti.js and backend vedic_graha_drishti.
 */
...
export function getHouseAspects(chartData, houseNum, rashiIndex) {
  const server =
    chartData?.graha_drishti_by_house?.[String(houseNum)]
    ?? chartData?.houses?.[houseNum - 1]?.graha_drishti;
  ...
  return getGrahaDrishtiToHouseSign(chartData, rashiIndex);
}
