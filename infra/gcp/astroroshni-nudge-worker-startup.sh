...
LOG_FILE="/var/log/astroroshni-nudge-worker-startup.log"

exec > >(tee -a "${LOG_FILE}") 2>&1

echo "========== AstroRoshni nudge worker startup $(date -u) =========="

exec 9>/var/lock/astroroshni-nudge-worker-startup.lock
flock -n 9 || exit 0
...
sudo -u "${APP_USER}" bash -lc "
  ...
  export UVICORN_LIMIT_CONCURRENCY=100
  bash deploy.sh
"

echo "========== AstroRoshni nudge worker startup complete $(date -u) =========="
