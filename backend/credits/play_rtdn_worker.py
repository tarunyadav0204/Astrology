...
from credits.routes import _sync_subscription_from_play
...
def _event_id_from_payload(payload: Dict[str, Any], message_id: str) -> str:
    sn = payload.get("subscriptionNotification") if isinstance(payload.get("subscriptionNotification"), dict) else {}
    ...
    parts = [token, product_id, ntype, et, str(message_id or "").strip()]
    return "|".join(parts)
...
def _process_one(
    *,
    ...
    if not isinstance(sub_notif, dict):
        logger.warning("Skipping non-subscription RTDN message_id=%s payload=%s", message_id, payload)
        return True

    purchase_token = str(sub_notif.get("purchaseToken") or "").strip()
    product_id = str(sub_notif.get("subscriptionId") or "").strip()
    notification_type = _safe_int(sub_notif.get("notificationType"))
