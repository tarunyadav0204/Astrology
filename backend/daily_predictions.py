...
class DailyPredictionEngine:
    ...
    def init_prediction_rules_db(self):
        """Initialize daily prediction rules database (Postgres)."""
        with get_conn() as conn:
            cur = execute(conn, "SELECT COUNT(*) FROM daily_prediction_rules")
            if cur.fetchone()[0] == 0:
                self._insert_basic_rules(conn)
            conn.commit()

    def _insert_basic_rules(self, conn) -> None:
        ...
        basic_rules = [
            ...
            },
            {
                ...
                "conditions": {"antar_dasha": "Mercury"},
            ...
            },
            {
                ...
                "conditions": {"jupiter_aspects": ["1st", "5th", "9th", "10th", "11th"]},
            ...
            },
            {
                ...
                "rule_type": "transit_aspect",
                "conditions": {"saturn_aspects": ["6th", "8th", "12th"]},
            ...
            },
            {
                ...
                "conditions": {"has_moon_nakshatra": True},
    ...
    def _extract_current_state(
        self, birth_data: Dict, chart_data: Dict, dasha_data: Dict, transit_data: Dict, target_date: date
    ) -> Dict:
        ...
        if transit_data and "planets" in transit_data:
            ...
            if "Jupiter" in transit_data["planets"]:
                jupiter_house = self._calculate_house_from_ascendant(
                    transit_data["planets"]["Jupiter"]["longitude"], chart_data.get("ascendant", 0)
                )
                jupiter_aspects = [str(jupiter_house)]

            if "Saturn" in transit_data["planets"]:
                saturn_house = self._calculate_house_from_ascendant(
                    transit_data["planets"]["Saturn"]["longitude"], chart_data.get("ascendant", 0)
                )
    ...
    def _find_matching_rules(self, current_state: Dict) -> List[Dict]:
        ...
        with get_conn() as conn:
            cur = execute(
                conn,
                """
                SELECT * FROM daily_prediction_rules
                WHERE COALESCE(is_active, 'TRUE') IN ('TRUE', 'true', '1', 't')
                """,
                (),
            )
    ...
    def reset_prediction_rules(self) -> bool:
        ...
        try:
            with get_conn() as conn:
                execute(conn, "DELETE FROM daily_prediction_rules")
                self._insert_basic_rules(conn)
                conn.commit()
            return True
        except Exception as e:
