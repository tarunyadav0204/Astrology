"""Read-only adapter for the compiled Health ontology policy bundle."""
...
DEFAULT_BUNDLE = (
    ...
    / "knowledge_graph"
    / "health_poc"
    / "health-runtime-preview.json"
)


@dataclass(frozen=True)
class HealthGraphPolicy:
    runtime_key: str
    ...
    @classmethod
    def from_dict(cls, runtime_key: str, value: Mapping[str, Any]) -> "HealthGraphPolicy":
        return cls(
...
class HealthGraphPolicyStore:
    def __init__(self, bundle_path: Path | str = DEFAULT_BUNDLE) -> None:
        ...
        self._policies = {
            key: HealthGraphPolicy.from_dict(key, value)
            for key, value in document["topics"].items()
        }

    def resolve(self, runtime_key: str) -> HealthGraphPolicy | None:
        return self._policies.get(runtime_key)

    def require(self, runtime_key: str) -> HealthGraphPolicy:
        policy = self.resolve(runtime_key)
        if policy is None:
            raise KeyError(f"Unknown Health ontology runtime key: {runtime_key}")
        return policy
...
@lru_cache(maxsize=1)
def default_health_graph_policy_store() -> HealthGraphPolicyStore:
    return HealthGraphPolicyStore()
