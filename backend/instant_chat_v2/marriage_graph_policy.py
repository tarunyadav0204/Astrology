"""Read-only adapter for the compiled Marriage and Relationship graph bundle."""
...
DEFAULT_BUNDLE = (
    ...
    / "knowledge_graph"
    / "marriage_poc"
    / "marriage-runtime-preview.json"
)


@dataclass(frozen=True)
class MarriageGraphPolicy:
    runtime_key: str
    ...
    @classmethod
    def from_dict(cls, runtime_key: str, value: Mapping[str, Any]) -> "MarriageGraphPolicy":
        return cls(
...
class MarriageGraphPolicyStore:
    def __init__(self, bundle_path: Path | str = DEFAULT_BUNDLE) -> None:
        ...
        self._policies = {
            key: MarriageGraphPolicy.from_dict(key, value)
            for key, value in document["topics"].items()
        }

    def resolve(self, runtime_key: str) -> MarriageGraphPolicy | None:
        return self._policies.get(runtime_key)

    def require(self, runtime_key: str) -> MarriageGraphPolicy:
        policy = self.resolve(runtime_key)
        if policy is None:
            raise KeyError(f"Unknown Marriage ontology runtime key: {runtime_key}")
        return policy
...
@lru_cache(maxsize=1)
def default_marriage_graph_policy_store() -> MarriageGraphPolicyStore:
    return MarriageGraphPolicyStore()
