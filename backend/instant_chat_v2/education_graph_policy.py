"""Read-only adapter for the compiled Education ontology bundle."""
...
DEFAULT_BUNDLE = Path(__file__).resolve().parents[2] / "knowledge_graph" / "education_poc" / "education-runtime-preview.json"


@dataclass(frozen=True)
class EducationGraphPolicy:
    runtime_key: str
    ...
    @classmethod
    def from_dict(cls, runtime_key: str, value: Mapping[str, Any]) -> "EducationGraphPolicy":
        return cls(
...
class EducationGraphPolicyStore:
    def __init__(self, bundle_path: Path | str = DEFAULT_BUNDLE) -> None:
        ...
        self._policies = {
            key: EducationGraphPolicy.from_dict(key, value)
            for key, value in document["topics"].items()
        }

    def resolve(self, runtime_key: str) -> EducationGraphPolicy | None:
        return self._policies.get(runtime_key)

    def require(self, runtime_key: str) -> EducationGraphPolicy:
        policy = self.resolve(runtime_key)
        if policy is None:
            raise KeyError(f"Unknown Education ontology runtime key: {runtime_key}")
        return policy
...
@lru_cache(maxsize=1)
def default_education_graph_policy_store() -> EducationGraphPolicyStore:
    return EducationGraphPolicyStore()
