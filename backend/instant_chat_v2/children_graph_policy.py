"""Read-only adapter for the compiled Children ontology bundle."""
...
DEFAULT_BUNDLE = Path(__file__).resolve().parents[2] / "knowledge_graph" / "children_poc" / "children-runtime-preview.json"


@dataclass(frozen=True)
class ChildrenGraphPolicy:
    runtime_key: str
    ...
    @classmethod
    def from_dict(cls, runtime_key: str, value: Mapping[str, Any]) -> "ChildrenGraphPolicy":
        return cls(
...
class ChildrenGraphPolicyStore:
    def __init__(self, bundle_path: Path | str = DEFAULT_BUNDLE) -> None:
        ...
        self._policies = {
            key: ChildrenGraphPolicy.from_dict(key, value)
            for key, value in document["topics"].items()
        }

    def resolve(self, runtime_key: str) -> ChildrenGraphPolicy | None:
        return self._policies.get(runtime_key)

    def require(self, runtime_key: str) -> ChildrenGraphPolicy:
        policy = self.resolve(runtime_key)
        if policy is None:
            raise KeyError(f"Unknown Children ontology runtime key: {runtime_key}")
        return policy
...
@lru_cache(maxsize=1)
def default_children_graph_policy_store() -> ChildrenGraphPolicyStore:
    return ChildrenGraphPolicyStore()
