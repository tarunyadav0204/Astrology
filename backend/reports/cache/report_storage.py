...
def get_latest_completed_report_job(
    userid: int,
    report_type: str,
    subject_hash: str,
    language: str,
    ...
    """Most recent completed job for this user/pair/language/version (for Open-existing UX)."""
    with get_conn() as conn:
        ensure_report_jobs_table(conn, execute_fn)
        cur = execute_fn(
            conn,
            """
            SELECT report_id, userid, report_type, subject_hash, language, status, request_json, result_data,
                   error_message, report_version, created_at, started_at, completed_at
            FROM report_generation_jobs
            WHERE userid = %s
              AND report_type = %s
              AND subject_hash = %s
...
def get_latest_active_report_job(
    userid: int,
    report_type: str,
    subject_hash: str,
    language: str,
    ...
    """Most recent pending/processing job for this user/pair/language/version."""
    with get_conn() as conn:
        ensure_report_jobs_table(conn, execute_fn)
        cur = execute_fn(
            conn,
