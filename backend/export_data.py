...
with open('nakshatras_data.sql', 'w') as f:
    ...
    for row in nakshatras:
        values = ', '.join([f"'{str(val).replace(chr(39), chr(39)+chr(39))}'" if val is not None else 'NULL' for val in row[1:]])  # Skip id
...
cursor.execute("SELECT * FROM planet_nakshatra_interpretations ORDER BY planet, nakshatra, house")
