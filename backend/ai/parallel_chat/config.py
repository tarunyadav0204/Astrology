...
- `ASTRO_PARALLEL_BRANCH_TIMEOUT_S` — max wall time per **non-critical** branch LLM call (default `480`). On timeout/error, that branch returns `status=unavailable` and merge continues.
- `ASTRO_PARALLEL_CRITICAL_TIMEOUT_S` — same for the **Parashari** branch (default `480`). This keeps the parallel chat path aligned with the 8-minute UI polling window while still capping SDK retry loops.
...
def parallel_branch_timeout_s(*, critical: bool) -> float:
    ...
    default = "480" if critical else "480"
