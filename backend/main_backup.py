...
async def calculate_chart_only(birth_data: BirthData, node_type: str = 'mean', current_user: User = Depends(get_current_user)):
    try:
        print(f"🔍 Chart-only calculation request for: {birth_data.name}")
        ...
        print(f"✅ Validation passed, proceeding with chart-only calculation")
        
        # Calculate chart without saving to database
        from calculators.chart_calculator import ChartCalculator
        calculator = ChartCalculator({})
        result = calculator.calculate_chart(birth_data, node_type)
        print(f"✅ Chart-only calculation completed successfully")
        return result
        ...
        print(f"❌ Chart-only calculation error: {error_details}")
...
async def calculate_all_charts(birth_data: BirthData, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        from calculators.chart_calculator import ChartCalculator
        calculator = ChartCalculator({})
        base_chart = calculator.calculate_chart(birth_data, 'mean')
...
async def calculate_dasha(birth_data: BirthData):
    print(f"🔍 [DASHA_DEBUG] /api/calculate-dasha endpoint called")
    print(f"🔍 [DASHA_DEBUG] Raw birth_data received: {birth_data.model_dump()}")
    result = await calculate_accurate_dasha(birth_data)
    print(f"🔍 [DASHA_DEBUG] Returning result from /api/calculate-dasha")
    return result
