...
# 2 workers + schema ensure can exceed ~40s when DB is busy; give cold start more room.
...
if [ "${SYNC_GCP_SECRETS}" = "true" ]; then
  ...
  overlay_gcp_secret_in_env_file "${NUDGE_AUDIENCE_DATABASE_SECRET_NAME}" "NUDGE_AUDIENCE_DATABASE_URL" "${BACKEND_DIR}/.env"
  overlay_gcp_secret_in_env_file "${NUDGE_NOTIFICATION_DATABASE_SECRET_NAME}" "NUDGE_NOTIFICATION_DATABASE_URL" "${BACKEND_DIR}/.env"
