...
def objects(block: str, predicate: str) -> list[str]:
    ...
    return re.findall(r"(?:career|health|marriage|wealth|education|children|home|foreign|domain|ar):[A-Za-z0-9_]+", match.group(1)) if match else []
...
def declared_resources(ttl: str) -> set[str]:
    return set(re.findall(r"(?m)(?:^|\.\s*)((?:career|health|marriage|wealth|education|children|home|foreign|domain|ar):[A-Za-z0-9_]+)\s+", ttl))
...
def validate_integrity(ttl: str, question_resources: list[str]) -> None:
    ...
    for resource in declared:
        ...
        if resource in question_resources:
            ...
            for reference in re.findall(r"(?:career|health|marriage|wealth|education|children|home|foreign|domain):[A-Za-z0-9_]+", block):
    ...
    capability_resources = re.findall(r"(?m)(?:^|\.\s*)((?:career|health|marriage|wealth|education|children|home|foreign):[A-Za-z0-9_]+)\s+a\s+ar:CalculatorCapability\b", ttl)
...
def main() -> int:
    domain_label = {
        ...
        "foreign_life_poc": "Travel, Relocation and Foreign Life",
