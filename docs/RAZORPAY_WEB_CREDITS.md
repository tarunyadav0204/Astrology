...
Web users buy credits through **Razorpay Checkout** (UPI, cards, netbanking). Live packs:

| Pack | Credits | Product ID | Default price |
|------|--------:|------------|---------------|
| Shuruaat Pack | 50 | `credits_50` | ₹99 |
| Aashirwad Pack | 100 | `credits_100` | ₹199 |
| Sadhak Pack | 250 | `credits_250` | ₹499 |
| Guru Pack | 999 (+50 bonus → 1049) | `credits_999` | ₹1999 |
...
| `RAZORPAY_PRICE_PAISE_50` etc. | No | Override default INR prices (values in **paise**: `9900` = ₹99). |

Defaults (paise): 50 → 9900, 100 → 19900, 250 → 49900, 999 → 199900. Override with `RAZORPAY_PRICE_PAISE_N` to match Play Console.
...
| POST | `/api/credits/razorpay/create-order` | Bearer | Body `{ "credits": 50 \| 100 \| 250 \| 999 }`. Creates Razorpay order; notes include `userid`, `credits`, `product_id`. |
...
`CreditsModal` loads `/api/credits/razorpay/catalog` when the user is signed in, shows packs, and opens `https://checkout.razorpay.com/v1/checkout.js` Checkout. No Razorpay secret in the frontend — only `key_id` from the API (same as publishing Key ID in client-side Checkout).
